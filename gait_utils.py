...
def _yolo_video_to_gei(video_path: str, yolo=None) -> np.ndarray:
    ...
    Handles both v2 (aspect-aware) and v1 (legacy square) pipelines based on
    ...
    try:
        while True:
            ...
            # Run YOLO inference focusing only on 'person' (class 0)
            results = yolo(frame, classes=[0], verbose=False, device=_device)
            
            for result in results:
                if _CONFIG.aspect_aware_align: 
                    # v2 path: Best-mask selection -> QC -> Aspect-aware alignment.
                    full_mask = _pre.silhouette_from_yolo_result(result, frame.shape[:2])
                    if full_mask is None: 
                        continue # Skip frame if no good mask found
                
                    # Use GEI_SIZE (64, 44) from the config sidecar.
                    aligned = _pre.aligned_silhouette_from_mask(full_mask, out_size=GEI_SIZE)
                    if aligned is not None:
                        silhouettes.append(aligned)
                
                else:
                    # v1 path: Legacy square-resize behavior.
                    if result.masks is not None:
                        # Grab the first mask and resize to original frame shape
                        mask = result.masks.data[0].cpu().numpy()
                        mask = cv2.resize(mask, (frame.shape[1], frame.shape[0]))
                        binary_mask = (mask * 255).astype(np.uint8)
                        
                        # Use the bounding box to crop and squash to square 64x64
                ...
                # We only process the primary person in the frame for gait
                break 

    finally:
        cap.release()

    # Minimum frame check to ensure we have enough data for a stable GEI
    if len(silhouettes) < _CONFIG.min_silhouette_frames:
        raise ValueError(f"Only {len(silhouettes)} silhouettes detected. Need more clear walking data.")

    if _CONFIG.aspect_aware_align:
        # Generate the average using the period-aware v2 logic.
        gei = _pre.build_gei(silhouettes, use_period_detection=_CONFIG.use_period_detection)
        if gei is None:
            raise ValueError("Failed to build aspect-aware GEI.")
        return gei
